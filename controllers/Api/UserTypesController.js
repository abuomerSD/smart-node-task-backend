...
exports.selectUserTypesByFilter=async (req,res,next)=>{
  var params = { limit: 10, page: 1, constrains: { name: "" }, 
  relations:[{model:conn.users},]
  }
  const result = await filter.filter("user_types", params)
  if(result){
...
exports.search = async (req, res, next) => {
  ...
  await conn.user_types.findAll({
    ...
    where: {
     ...
     var count = conn.user_types.findAll();
...
exports.getUserTypes =async (req, res, next) => {
    try{
      const result=await conn.user_types.findAll()
       if(result.length!=0)
         res.status(200).json({ status: true, data: result })
         else{
           res.status(200).json({ status: false, msg: `No data founded` })
...
exports.createUserTypes =async (req, res, next) => {
    try{
      const result=await conn.user_types.create(req.body)
       if(result.length!=0)
         res.status(200).json({ status: true, data: result })
         else{
           res.status(200).json({ status: false, msg: `No data founded` })
...
exports.paginate = async (req, res, next) => {
 try {
    ...
    var result = await conn.user_types.findAll({
      order: [["id", "DESC"]],
      include:['users',],
      offset: offset,
      limit: req.body.limit,
    ...
    if (result) {
      var count = await conn.user_types.findAll();
...
//@route  GET
//@access Public
exports.getUserTypesById =async (req, res, next) => {
  try{
      const result=await conn.user_types.findOne({where:{id:req.params.id}})
      if(result.length!=0)
         res.status(200).json({ status: true, data: result })
         else{
           res.status(200).json({ status: false, msg: `No data founded` })
...
exports.updateUserTypes =async (req, res, next) => {
  try{
    const isUpdated=await conn.user_types.update(req.body,{where:{id:req.params.id}})
    if(isUpdated)
...
//@route  Delete
//@access Public
exports.deleteUserTypes = async (req, res, next) => {
   try{
    const isDeleted=await conn.user_types.destroy({where:{id:req.params.id}})
    if(isDeleted)
