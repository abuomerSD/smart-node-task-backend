...
exports.selectUserRolesByFilter=async (req,res,next)=>{
  var params = { limit: 10, page: 1, constrains: { name: "" }, 
  
  }
  const result = await filter.filter("user_roles", params)
  if(result){
...
exports.pagination = async (req, res, next) => {
 try {
    ...
    var result = await conn.user_roles.findAll({
      order: [["id", "DESC"]],
      include:[],
      offset: offset,
      limit: req.body.limit,
    ...
    if (result) {
      var count = await conn.user_roles.findAll();
...
exports.getUserRolesById =async (req, res, next) => {
  try{
      const result=await conn.user_roles.findOne({where:{id:req.params.id}})
      if(result.length!=0)
         res.status(200).json({ status: true, data: result })
         else{
           res.status(200).json({ status: false, msg: `No data founded` })
...
exports.updateUserRoles =async (req, res, next) => {
  try{
    const isUpdated=await conn.user_roles.update(req.body,{where:{id:req.params.id}})
    if(isUpdated)
...
exports.deleteUserRoles = async (req, res, next) => {
   try{
    const isDeleted=await conn.user_roles.destroy({where:{id:req.params.id}})
    if(isDeleted)
