...
exports.search = async (req, res, next) =>
{
    ...
    {
        var count = await conn.categories.count({
            where: {
                name: {
                    [Op.like]: '%' + search + '%'
                }
...
exports.pagination = async (req, res, next) =>
{
    try
    {
        ...
        var result = await conn.categories.findAll({
            order: [["id", "DESC"]],
            include: [],
            offset: offset,
...
exports.getCategoriesById = async (req, res, next) =>
{
    try
    {
        const result = await conn.categories.findOne({ where: { id: req.params.id } })
        if (result.length != 0)
            res.status(200).json({ status: true, data: result })
        else
        {
...
exports.updateCategories = async (req, res, next) =>
{
    try
    {
        const isUpdated = await conn.categories.update(req.body, { where: { id: req.params.id } })
        if (isUpdated)
...
exports.deleteCategories = async (req, res, next) =>
{
    try
    {
        const isDeleted = await conn.categories.destroy({ where: { id: req.params.id } })
        if (isDeleted)
