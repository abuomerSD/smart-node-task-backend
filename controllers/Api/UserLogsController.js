...
exports.selectUserLogsByFilter=async (req,res,next)=>{
  var params = { limit: 10, page: 1, constrains: { name: "" }, 
  
  }
  const result = await filter.filter("user_logs", params)
  if(result){
...
exports.search = async (req, res, next) => {
  ...
  await conn.user_logs.findAll({
    ...
    where: {
     ...
     var count = conn.user_logs.findAll();
...
exports.getUserLogs =async (req, res, next) => {
    try{
      const result=await conn.user_logs.findAll()
       if(result.length!=0)
         res.status(200).json({ status: true, data: result })
         else{
           res.status(200).json({ status: false, msg: `No data founded` })
...
exports.createUserLogs =async (req, res, next) => {
    try{
      const result=await conn.user_logs.create(req.body)
       if(result.length!=0)
         res.status(200).json({ status: true, data: result })
         else{
           res.status(200).json({ status: false, msg: `No data founded` })
...
exports.pagination = async (req, res, next) => {
 try {
    ...
    var result = await conn.user_logs.findAll({
      order: [["id", "DESC"]],
      include:[],
      offset: offset,
      limit: req.body.limit,
    ...
    if (result) {
      var count = await conn.user_logs.findAll();
...
//@route  GET
//@access Public
exports.getUserLogsById =async (req, res, next) => {
  try{
      const result=await conn.user_logs.findOne({where:{id:req.params.id}})
      if(result.length!=0)
         res.status(200).json({ status: true, data: result })
         else{
           res.status(200).json({ status: false, msg: `No data founded` })
...
//@route  Put
//@access Public
exports.updateUserLogs =async (req, res, next) => {
  try{
    const isUpdated=await conn.user_logs.update(req.body,{where:{id:req.params.id}})
    if(isUpdated)
...
//@route  Delete
//@access Public
exports.deleteUserLogs = async (req, res, next) => {
   try{
    const isDeleted=await conn.user_logs.destroy({where:{id:req.params.id}})
    if(isDeleted)
